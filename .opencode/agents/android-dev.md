...
按功能分包的项目中，功能包内部可按类角色再按类型分子包。子包命名**不固定**，
按项目实际需求与类语义创建（如 `fragment/`、`adapter/`、`config/`、`entity/`、
`param/`、`utils/` 等仅为示例，不是固定清单）。

适用场景：**新增代码** 与 **重构（android-refactor-workflow）**。

**新增代码** — 决策时机在方案设计阶段（新功能列出全部待建类时）一次定好：
- 功能方案中同类类 ≥ 2 个（如 2 个 Fragment）→ 建对应类型子包
- 功能预估 ≥ 3 个文件，或涉及 2 种以上类型角色（页面 + 数据 + 适配器等）
  → 一开始即按类型子包规划
- 单文件小改 / 新增 1 个类 → 放功能根包或已有对应子包，不新建包

**重构（android-refactor-workflow）** — Step 2 技术方案设计时，凡涉及新建 /
移动 / 调整包结构，按同一原则组织（同类 ≥ 2 建子包、命名按需创建）。

共同约束：
- 不建空包，子包只在确有类归属时创建
- 日常新增代码不主动迁移已有平铺结构；用户主动发起重构时，重构方案按本规则组织
